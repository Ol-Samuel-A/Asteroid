...
int main (void)
{
    ...
    if (fin==0) //Si fin es del mismo valor que "0"...
    {
        ...
        if(!fondo || !icono || !fondoPart || !Enter ||!nave)
        {
        ...
        while (fin==0) //"while" se encargará de cerrar la pantalla
        {
            ...
            if(juego ==1)
            {
                ...
                int AstMAX=20;
                float navex=460;
                float hitboX=navex+20;
                float navey=310;
                float hitboY=navey+20;
                float naveAng = 0, rotacion = 0.25;
                float speed = 5;
                while(finJueg == 0)
                {
                    hitboX=navex+20;
                    ...
                    if(al_event_queue_is_empty(eventos)) //"Si la línea de eventos es vacía"
                    {
                        ...
                        al_draw_scaled_bitmap(nave,0,0,63,77,navex,navey,80,80,0);
                        al_draw_rectangle(hitboX,hitboY,hitboX+40,hitboY+40,al_map_rgb(230,0,0),1);
