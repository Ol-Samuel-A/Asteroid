...
int main (void)
{
    ...
    if (fin==0) //Si fin es del mismo valor que "0"...
    {
        ...
        while (fin==0) //"while" se encargará de cerrar la pantalla
        {
            ...
            {
                char PantallaF = 0;
                float spawnAst = 0,TempSuma = 0.02;
                ...
                while(PantallaF == 1)
                {
                    ...
                    {
                        al_draw_scaled_bitmap(fondoPart,0,0,728,410,0,0,1000,700,0);
                        al_draw_text(fuenteSecc,al_map_rgb(255,255,255),530,225,0,"Perdiste ...");
                        al_draw_text(fuenteSubt, al_map_rgb(255, 255, 0), 450, 300, 0, DibujoScore);
                        al_draw_rectangle(350,450,650,550,al_map_rgb(255,255,255),0);
                        al_draw_text(fuenteSubt,al_map_rgb(255,255,255),350,560,0,"Nombre(max 10 caracteres)");
                    }
                    al_flip_display();
                    switch(evento.type)
                    {
